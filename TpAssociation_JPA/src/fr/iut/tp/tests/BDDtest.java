...
public class BDDtest {
	...
	public static void main(String[] args) {
		...
		Adherent leAdh = adhDAO.create(0, "log", "pwd", "prenom", "nom",
				"adress", 44830, "city", "country");
		...
		for (Commande uneCom : listCom) {
			if (uneCom.getAdherent().getAdherent_id() == 0) {
				System.out.println("Commande n°" + uneCom.getCommande_Id()
						+ " venant de " + uneCom.getAdherent().getNom() + " "
						+ uneCom.getAdherent().getPrenom());
				List<Article> artCommande = uneCom.getArticles();
				for (Article art : artCommande) {
					System.out.println(art.getCode() + " " + art.getNom() + " "
							+ art.getPrix());
				}
			}
